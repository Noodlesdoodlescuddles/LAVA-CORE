🔥 PROJECT LAVACORE — POTATO ASCENSION v5.1 🥔📈
...
WHAT IT DOESN'T DO:
LAVA-CORE does NOT magically create extra hardware performance. It reduces visual workload so weaker devices may have more rendering headroom. Actual FPS improvements depend on the game, device, graphics settings, and what is causing the slowdown.

⚠️ COMPATIBILITY:
Because LAVA-CORE changes client-side visual properties, some games may look different or may not benefit much from it. Some games may also recreate effects, so LAVA-CORE continuously checks for them.
